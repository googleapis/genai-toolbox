...
func TestToolsetEndpoint(t *testing.T) {
	...
	toolsMap, toolsets, _, _, _, _ := testutils.SetUpPrimitives(t, mockTools, nil, nil, nil)
	r, shutdown := setUpServer(t, "api", toolsMap, toolsets, nil, nil, nil, nil)
	defer shutdown()
...
func TestApiRequestBodyLimitOverride(t *testing.T) {
	...
	r, shutdown := setUpServer(t, "api", toolsMap, toolsets, nil, nil, nil, nil, withHTTPMaxRequestBytes(customLimit))
	defer shutdown()
	ts := runServer(r, false)
	defer ts.Close()
