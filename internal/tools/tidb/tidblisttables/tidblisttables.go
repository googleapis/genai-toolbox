...
const resourceType string = "tidb-list-tables"

// listTablesStatement queries table metadata from TiDB's INFORMATION_SCHEMA.
// TiDB is MySQL-compatible but has some differences in system tables.
// This query provides detailed schema information including TiDB-specific features.
            ...
            ELSE
                CONVERT(
                    JSON_OBJECT(
                        ...
                        'tiflash_replica_count', IFNULL(T.TIFLASH_REPLICA_COUNT, 0),
                        'columns', (
                            SELECT
                                IFNULL(
                                    JSON_ARRAYAGG(
...
func (cfg Config) Initialize(srcs map[string]sources.Source) (tools.Tool, error) {
	allParameters := parameters.Parameters{
		...
		parameters.NewStringParameterWithDefault("output_format", "detailed", "Optional: Use 'simple' for names only or 'detailed' for full info including TiFlash replica count."),
	}
...
func (t Tool) Invoke(ctx context.Context, resourceMgr tools.SourceProvider, params parameters.ParamValues, accessToken tools.AccessToken) (any, util.ToolboxError) {
	...
	if !ok {
		return nil, util.NewAgentError(fmt.Sprintf("invalid 'table_names' parameter; expected a string but got %T", paramsMap["table_names"]), nil)
	}
