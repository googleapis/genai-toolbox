...
func TestQueryClassifier_Classify(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedType:  ReadQuery,
			expectedRead:  []string{"MATCH", "WHERE", "RETURN", "ORDER BY", "SKIP", "LIMIT"},
			expectedWrite: []string{},
			minConfidence: 1.0,
		},
		{
		...
		},
		{
			...
			expectedType:  WriteQuery,
			expectedWrite: []string{"DETACH DELETE"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.expectedType, result.Type, "Query type mismatch")
			assert.GreaterOrEqual(t, result.Confidence, tt.minConfidence, "Confidence too low")
			assert.ElementsMatch(t, tt.expectedWrite, result.WriteTokens, "Write tokens mismatch")
			assert.ElementsMatch(t, tt.expectedRead, result.ReadTokens, "Read tokens mismatch")
		})
	}
}
...
func TestQueryClassifier_AbuseCases(t *testing.T) {
	...
	tests := []struct {
		...
		query        string
		expectedType QueryType
		// We check for a "best effort" token extraction, not perfect parsing.
		expectedWrite []string
		expectedRead  []string
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// The primary goal is to ensure the classifier doesn't panic.
