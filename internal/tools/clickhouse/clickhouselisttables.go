...
func (t ListTablesTool) Invoke(ctx context.Context, params tools.ParamValues) ([]any, error) {
	...
	// Build the SQL query based on whether table names are provided
	var query string
	var args []interface{}

	if strings.TrimSpace(tableNamesParam) == "" {
		// Query for all tables in current database when no specific table names are provided
		query = `
			SELECT
		...
		// Parse comma-separated table names and build IN clause
		...
		if len(trimmedNames) == 0 {
			// If no valid table names after trimming, treat as empty and query all tables
		...
		// Use args directly since we only have one WHERE clause now
	}
