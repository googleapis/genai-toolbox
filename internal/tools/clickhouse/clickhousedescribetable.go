...
func (cfg DescribeTableConfig) Initialize(srcs map[string]sources.Source) (tools.Tool, error) {
	...
	databaseParameter := tools.NewStringParameterWithDefault("database_name", "", "Optional: The database name. If empty, uses the current database.")
	parameters := tools.Parameters{tableParameter, databaseParameter}
...
func (t DescribeTableTool) Invoke(ctx context.Context, params tools.ParamValues) (any, error) {
	...
	database, ok := sliceParams[1].(string)
	if !ok {
		return nil, fmt.Errorf("unable to cast database_name parameter %v", sliceParams[1])
	}

	// Use current database if none specified
	var databaseCondition string
	var args []interface{}
	if strings.TrimSpace(database) == "" {
		databaseCondition = "database = currentDatabase()"
		args = []interface{}{table}
	} else {
		databaseCondition = "database = ?"
		args = []interface{}{table, strings.TrimSpace(database)}
	}
	...
	tableRows, err := t.Pool.QueryContext(ctx, tableQuery, args...)
	if err != nil {
	...
	columnRows, err := t.Pool.QueryContext(ctx, columnsQuery, args...)
	if err != nil {
