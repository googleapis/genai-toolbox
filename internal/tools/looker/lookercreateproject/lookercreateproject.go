...
const resourceType string = "looker-create-project"
...
func (cfg Config) Initialize(srcs map[string]sources.Source) (tools.Tool, error) {
	...
	gitReleaseMgmtEnabledParameter := parameters.NewBooleanParameterWithRequired("git_release_mgmt_enabled", "If true, advanced git release management is enabled for this project.", false)
	allowWarningsParameter := parameters.NewBooleanParameterWithRequired("allow_warnings", "Validation policy: If true, the project can be committed with warnings when validation_required is true.", false)
...
func (t Tool) Invoke(ctx context.Context, resourceMgr tools.SourceProvider, params parameters.ParamValues, accessToken tools.AccessToken) (any, util.ToolboxError) {
	...
	resp, err := sdk.CreateProject(body, source.LookerApiSettings())
	if err != nil {
		return nil, util.NewClientServerError(fmt.Sprintf("error making create_project request: %s", err), http.StatusInternalServerError, err)
	}
	return resp, nil
}
