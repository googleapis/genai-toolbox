...
import (
	...
	"github.com/googleapis/genai-toolbox/internal/tools/opengemini/opengeminisql"
)
...
func TestParseFromYamlOpenGemini(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			`,
			want: server.ToolConfigs{
				"example_tool": opengeminisql.Config{
					Name:         "example_tool",
					Kind:         "opengemini-sql",
					Source:       "my-opengemini-instance",
					Description:  "some description",
...
func TestParseFromYamlWithTemplateParamsOpenGemini(t *testing.T) {
	ctx, err := testutils.ContextWithNewLogger()
	if err != nil {
		t.Fatalf("unexpected error: %s", err)
	}
