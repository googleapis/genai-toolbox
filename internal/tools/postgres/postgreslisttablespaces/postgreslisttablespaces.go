...
const listTableSpacesStatement = `
	...
	tablespace_info AS (
		SELECT
		spcname AS name,
		pg_catalog.pg_get_userbyid(spcowner) AS owner_username,
		CASE
	...
	FROM
		tablespace_info
	WHERE
		($1::text IS NULL OR name LIKE '%' || $1::text || '%')
	ORDER BY
		name
...
func (cfg Config) Initialize(srcs map[string]sources.Source) (tools.Tool, error) {
	...
	allParameters := parameters.Parameters{
		parameters.NewStringParameterWithDefault("name", "", "Optional: a text to filter results by tablespace name. The input is used within a LIKE clause."),
...
func (t Tool) Invoke(ctx context.Context, params parameters.ParamValues, accessToken tools.AccessToken) (any, error) {
	...
	duration, ok := paramsMap["name"].(string)
	if !ok {
		return nil, fmt.Errorf("invalid 'name' parameter; expected a string")
	}
	...
	results, err := t.pool.Query(ctx, listTableSpacesStatement, duration, limit)
	if err != nil {
