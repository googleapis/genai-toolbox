...
func BatchLogsURLFromProto(batchPb *dataprocpb.Batch) (string, error) {
	...
	timeMinus1Min := createTime.Add(-1 * time.Minute)
	timePlus10Min := stateTime.Add(10 * time.Minute)
	return BatchLogsURL(projectID, location, batchID, timeMinus1Min, timePlus10Min), nil
}
...
resource.labels.batch_id="%s"`
	...
	if !endTime.IsZero() {
		advancedFilter += fmt.Sprintf("\ntimestamp<=\"%s\"", endTime.Format(time.RFC3339Nano))
	}
