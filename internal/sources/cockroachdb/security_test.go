...
func TestApplyQueryLimits(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "existing LIMIT delimited by newline",
			sql:            "SELECT * FROM users\nLIMIT 50",
			maxRowLimit:    100,
		...
		{
			name:           "existing LIMIT delimited by tab",
			sql:            "SELECT * FROM users\tLIMIT 50",
			maxRowLimit:    100,
			expectedSQL:    "SELECT * FROM users\tLIMIT 50",
			shouldAddLimit: false,
		...
		{
			name:           "existing LIMIT in a multiline query",
			sql:            "SELECT *\nFROM users\nORDER BY id\nLIMIT 50",
			maxRowLimit:    100,
			expectedSQL:    "SELECT *\nFROM users\nORDER BY id\nLIMIT 50",
			shouldAddLimit: false,
		...
		{
			name:           "existing LIMIT with newline before the value",
