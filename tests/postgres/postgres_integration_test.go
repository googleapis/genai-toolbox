...
func runPostgresListViewsTest(t *testing.T, ctx context.Context, pool *pgxpool.Pool, tableName string) {
	viewName1 := "test_view_" + strings.ReplaceAll(uuid.New().String(), "-", "")
	viewName2 := "test_view_" + strings.ReplaceAll(uuid.New().String(), "-", "")
	dropViewfunc1 := setUpPostgresViews(t, ctx, pool, viewName1, tableName)
	defer dropViewfunc1()
	dropViewfunc2 := setUpPostgresViews(t, ctx, pool, viewName2, tableName)
	defer dropViewfunc2()
	...
	invokeTcs := []struct {
		...
		{
			name:           "invoke list_views",
			requestBody:    bytes.NewBuffer([]byte(`{}`)),
			wantStatusCode: http.StatusOK,
			want:           fmt.Sprintf(`[{"schemaname":"public","viewname":"%s","viewowner":"postgres"},{"schemaname":"public","viewname":"%s","viewowner":"postgres"}]`, viewName1, viewName2),
		},
		{
			name:           "invoke list_views with viewname1",
