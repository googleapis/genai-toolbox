...
func RunPostgresListViewsTest(t *testing.T, ctx context.Context, pool *pgxpool.Pool) {
	...
	invokeTcs := []struct {
	...
	}{
		{
			...
			requestBody:    bytes.NewBuffer([]byte(fmt.Sprintf(`{"viewname": "%s"}`, viewName))),
			wantStatusCode: http.StatusOK,
			want:           fmt.Sprintf(`[{"schemaname":"public","viewname":"%s","viewowner":"postgres","definition":" SELECT 1 AS col;"}]`, viewName),
		},
		{
