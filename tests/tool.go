...
func RunToolInvokeTest(t *testing.T, select1Want string, options ...InvokeTestOption) {
	...
	invokeTcs := []struct {
		...
		},
		{
			...
			requestHeader:  map[string]string{},
			requestBody:    bytes.NewBuffer([]byte(`{}`)),
			wantBody:       `{"error":"provided parameters were invalid: parameter \"id\" is required"}`,
			wantStatusCode: http.StatusOK,
		},
		{
			...
			wantBody:       `{"error":"provided parameters were invalid: parameter \"name\" is required"}`,
			wantStatusCode: http.StatusOK,
		},
		{
...
func RunRequest(t *testing.T, method, url string, body io.Reader, headers map[string]string) (*http.Response, []byte) {
	...
	if strings.Contains(url, "/api/tool/") && strings.Contains(url, "/invoke") {
		...
		var respBytes []byte
		if err != nil {
			// Tests interpret errors via exact HTTP status generated by MCP, wrapping the string via {"error":...} natively.
			respBytes = []byte(fmt.Sprintf(`{"error":%q}`, err.Error()))
		} else {
		...
		return &http.Response{StatusCode: status}, respBytes
	}
