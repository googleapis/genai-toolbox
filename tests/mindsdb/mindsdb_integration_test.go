...
func TestMindsDBToolEndpoints(t *testing.T) {
	...
	t.Run("mindsdb_core_functionality", func(t *testing.T) {
		tests.RunMCPToolInvokeSimpleTest(t, "my-simple-tool", select1Want)
		tests.RunMCPToolInvokeParametersTest(t, "my-exec-sql-tool", []byte(`{"sql": "SELECT 1"}`), select1Want)
		tests.RunMCPToolInvokeParametersTest(t, "my-exec-sql-tool", []byte(`{"sql": "SELECT 1+1 as result"}`), "[{\"result\":2}]")
		tests.RunMCPToolInvokeParametersTest(t, "my-exec-sql-tool", []byte(`{"sql": "SELECT 'hello' as greeting"}`), "[{\"greeting\":\"hello\"}]")
	})

	t.Run("mindsdb_sql_tests", func(t *testing.T) {
		tests.RunMCPToolInvokeParametersTest(t, "my-exec-sql-tool", []byte(`{"sql": "SELECT 1"}`), select1Want)
		tests.RunMCPToolInvokeParametersTest(t, "my-exec-sql-tool", []byte(`{"sql": "SHOW DATABASES"}`), "")
		tests.RunMCPToolInvokeParametersTest(t, "my-exec-sql-tool", []byte(`{"sql": "SHOW TABLES"}`), "")
		tests.RunMCPToolInvokeParametersTest(t, "my-exec-sql-tool", []byte(`{"sql": "SELECT TABLE_NAME FROM information_schema.TABLES LIMIT 1"}`), "")
		tests.RunMCPToolInvokeParametersTest(t, "my-exec-sql-tool", []byte(`{"sql": "SELECT 1+1 as result"}`), "")
		tests.RunMCPToolInvokeParametersTest(t, "my-exec-sql-tool", []byte(`{"sql": "SELECT UPPER('hello') as result"}`), "")
		tests.RunMCPToolInvokeParametersTest(t, "my-exec-sql-tool", []byte(`{"sql": "SELECT NOW() as current_time"}`), "")
	})

	// Test CREATE DATABASE (MindsDB's federated database capability)
	t.Run("mindsdb_create_database", func(t *testing.T) {
		tests.RunMCPToolInvokeParametersTest(t, "my-exec-sql-tool",
	...
	t.Run("mindsdb_error_handling", func(t *testing.T) {
		reqBytes1, _ := json.Marshal(map[string]any{
			"jsonrpc": "2.0",
			"id":      "1",
			"method":  "tools/call",
			"params": map[string]any{
				"name":      "my-exec-sql-tool",
				"arguments": map[string]any{"sql": "INVALID SQL QUERY"},
			},
		})
		resp, err := http.Post("http://127.0.0.1:5000/mcp", "application/json", bytes.NewBuffer(reqBytes1))
		if err != nil {
			t.Fatalf("error when sending request: %s", err)
		}
		defer resp.Body.Close()
		// Expect 200 HTTP, but an MCP-level error response
		if resp.StatusCode != http.StatusOK {
			t.Logf("Expected 200 for MCP layer, got %d", resp.StatusCode)
		}

		// Test empty SQL - expect this to trigger error
		reqBytes2, _ := json.Marshal(map[string]any{
			"jsonrpc": "2.0",
			"id":      "2",
			"method":  "tools/call",
			"params": map[string]any{
		...
		resp2, err := http.Post("http://127.0.0.1:5000/mcp", "application/json", bytes.NewBuffer(reqBytes2))
		if err != nil {
			t.Fatalf("error when sending request: %s", err)
		}
		defer resp2.Body.Close()
		if resp2.StatusCode != http.StatusOK {
			t.Logf("Expected 200 for empty SQL over MCP layer, got %d", resp2.StatusCode)
		}
	})

	// Test authentication - these are expected to fail but exercise auth code paths
	t.Run("mindsdb_auth_tests", func(t *testing.T) {
		reqBytes3, _ := json.Marshal(map[string]any{
			"jsonrpc": "2.0",
			"id":      "3",
			"method":  "tools/call",
			"params": map[string]any{
		...
		resp, err := http.Post("http://127.0.0.1:5000/mcp", "application/json", bytes.NewBuffer(reqBytes3))
		if err != nil {
			t.Fatalf("error when sending request: %s", err)
		}
		defer resp.Body.Close()
