...
func runBigQueryExecuteSqlToolInvokeDryRunTest(t *testing.T, datasetName, tableNameParam string) {
	...
	invokeTcs := []struct {
	...
	}{
		{
			...
			api:           "http://127.0.0.1:5000/api/tool/my-exec-sql-tool/invoke",
			requestHeader: map[string]string{},
			requestBody:   bytes.NewBuffer([]byte(`{"sql":"SELECT 1", "dryRun": true}`)),
		...
		},
		{
			...
			api:           "http://127.0.0.1:5000/api/tool/my-exec-sql-tool/invoke",
			requestHeader: map[string]string{},
			requestBody:   bytes.NewBuffer([]byte(fmt.Sprintf(`{"sql":"CREATE TABLE %s (id INT64, name STRING)", "dryRun": true}`, newTableName))),
