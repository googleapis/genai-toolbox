...
func runDataplexSearchAspectTypesToolInvokeTest(t *testing.T, aspectTypeId string) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			requestHeader:  map[string]string{},
			requestBody:    bytes.NewBuffer([]byte(fmt.Sprintf("{\"query\":\"name:projects/%s/locations/us/aspectTypes/%s\"}", DataplexProject, aspectTypeId))),
			wantStatusCode: 200,
			expectResult:   true,
		...
		},
		{
			...
			api:            "http://127.0.0.1:5000/api/tool/my-dataplex-search-aspect-types-tool/invoke",
			requestHeader:  map[string]string{},
			requestBody:    bytes.NewBuffer([]byte(fmt.Sprintf("{\"query\":\"name:projects/%s/locations/us/aspectTypes/non_existent_aspectType\"}", DataplexProject))),
			wantStatusCode: 200,
			expectResult:   false,
		},
		{
