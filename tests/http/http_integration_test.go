...
func getMcpAuthToolsConfig(jwksURL string) map[string]any {
	return map[string]any{
		"sources": map[string]any{},
		"authServices": map[string]any{
			"my-generic-auth": map[string]any{
				...
				"authorizationServer": jwksURL,
				"scopesRequired":      []string{"read:files"},
				"mcpEnabled":          true, // This ensures MCP global Auth applies
			},
		},
		"tools": map[string]any{},
	}
...
func TestMcpAuthEndpoints(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute)
	defer cancel()
	...
	toolsFile := getMcpAuthToolsConfig(jwksServer.URL)
	cmd, cleanup, err := tests.StartCmd(ctx, toolsFile, "--port", "5001")
	if err != nil {
		t.Fatalf("command initialization returned an error: %s", err)
	}
	defer cleanup()
	...
	api := "http://127.0.0.1:5001/mcp/sse"
	...
	t.Run("200 OK unauthenticated root endpoint", func(t *testing.T) {
		req, _ := http.NewRequest(http.MethodGet, "http://127.0.0.1:5001/", nil)
		resp, err := http.DefaultClient.Do(req)
		if err != nil {
			t.Fatalf("unable to send request: %s", err)
		}
