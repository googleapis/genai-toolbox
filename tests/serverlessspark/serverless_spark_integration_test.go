	...
	for _, tc := range tcs {
		t.Run(tc.name, func(t *testing.T) {
			...
			for i := 0; i < tc.numPages; i++ {
				...
				resp, err := invokeListBatches(request)
				if err != nil {
...
func invokeListBatches(request map[string]any) (*http.Response, error) {
	requestBytes, err := json.Marshal(request)
	if err != nil {
		return nil, fmt.Errorf("failed to marshal request: %w", err)
	}

	req, err := http.NewRequest(http.MethodPost, "http://127.0.0.1:5000/api/tool/list-batches/invoke", bytes.NewBuffer(requestBytes))
	if err != nil {
