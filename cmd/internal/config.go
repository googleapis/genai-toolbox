...
func (p *ConfigParser) ParseConfig(ctx context.Context, raw []byte) (Config, error) {
	...
	// Parse contents
	config.Sources, config.AuthServices, config.EmbeddingModels, config.Tools, config.Toolsets, config.Prompts, config.Promptsets, config.PiiPolicies, err = server.UnmarshalResourceConfig(ctx, raw)
	if err != nil {
		return config, err
	}
	return config, nil
...
func mergeConfigs(files ...Config) (Config, error) {
	...
	for fileIndex, file := range files {
		...
		// Check for conflicts and merge PII policies
