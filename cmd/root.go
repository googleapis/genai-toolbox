...
func NewCommand(opts ...Option) *Command {
	...
	baseCmd.AddCommand(invoke.NewCommand(invoke.Dependencies{
		Cfg:        &cmd.cfg,
		Out:        cmd.outStream,
		Err:        cmd.errStream,
		Version:    versionString,
		LoadConfig: cmd.loadAndMergeConfig,
		Setup:      cmd.Setup,
	}))
...
func (cmd *Command) loadAndMergeConfig(ctx context.Context) error {
	logger, err := util.LoggerFromContext(ctx)
	if err != nil {
		return err
	}
...
func run(cmd *Command) error {
	...
	if err := cmd.loadAndMergeConfig(ctx); err != nil {
		return err
	}
