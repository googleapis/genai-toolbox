...
type ToolsFile struct {
	...
	RoleDefinitions map[string]RoleDefinition `yaml:"roleDefinitions"`
	Tools           server.ToolConfigs        `yaml:"tools"`
	Toolsets        server.ToolsetConfigs     `yaml:"toolsets"`
}

// RoleDefinition defines the structure for role-based access control
...
func mergeToolsFiles(files ...ToolsFile) (ToolsFile, error) {
	...
	for fileIndex, file := range files {
		...
		// Check for conflicts and merge roleDefinitions
		for name, roleDef := range file.RoleDefinitions {
			if _, exists := merged.RoleDefinitions[name]; exists {
				conflicts = append(conflicts, fmt.Sprintf("roleDefinition '%s' (file #%d)", name, fileIndex+1))
			} else {
				merged.RoleDefinitions[name] = roleDef
			}
		}
	...
	if len(conflicts) > 0 {
		return ToolsFile{}, fmt.Errorf("resource conflicts detected:\n  - %s\n\nPlease ensure each source, authService, roleDefinition, tool, and toolset has a unique name across all files", strings.Join(conflicts, "\n  - "))
	}
