...
{{< notice note >}}
Prompts can be scoped to a collection using [Groups](../groups/). Connecting to a group's endpoint returns only that group's prompts from `prompts/list`; the default endpoint returns all prompts.
{{< /notice >}}
