...
{{< notice note >}}
A toolset is a tools-only [Group](../groups/), and Toolbox now loads every `kind: toolset` as a group. Existing toolsets keep working, with two exceptions: a `description` written on a toolset is dropped (with a warning), and reusing one name for both a `kind: toolset` and a `kind: group` is now a duplicate-name error. We recommend migrating to `kind: group` — even for tools-only collections — because a group lets you attach a `description` and scope other MCP primitives such as **prompts** alongside your **tools**. Run [`toolbox migrate`](../../../reference/cli.md) to convert automatically.
{{< /notice >}}
