...
| collection      | string   | false         | The name of the MongoDB collection to run the aggregation on. If omitted, it must be supplied at runtime as a `collection` parameter, which can be restricted using `allowedValues`. |
