...
| collection    | string   | false         | The name of the MongoDB collection from which to delete documents. If omitted, it must be supplied at runtime as a `collection` parameter, which can be restricted using `allowedValues`. |
