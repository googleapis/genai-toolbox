...
| collection  | string   | false         | The name of the MongoDB collection into which the document will be inserted. If omitted, it must be supplied at runtime as a `collection` parameter, which can be restricted using `allowedValues`. |
