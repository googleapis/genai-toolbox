...
| collection    | string   | false         | The name of the MongoDB collection in which to update documents. If omitted, it must be supplied at runtime as a `collection` parameter, which can be restricted using `allowedValues`. |
