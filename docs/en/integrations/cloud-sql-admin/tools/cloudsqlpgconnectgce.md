---
title: cloud-sql-postgres-connect-gce
type: docs
weight: 12
description: Connect a Cloud SQL for PostgreSQL instance to a Google Compute Engine VM — validates network reachability, recommends a connection method, and emits setup steps plus an optional code snippet.
---

## About

The `cloud-sql-postgres-connect-gce` tool helps an agent (or user) wire up a
Cloud SQL for PostgreSQL instance to a Google Compute Engine VM. It:
...
tools:
  connect_to_gce:
    ...
    description: Help me connect a Cloud SQL PostgreSQL instance to a GCE VM.
```
...
| ----------- | :------: | :----------: | ------------------------------------------------ |
| type        |  string  |     true     | Must be `cloud-sql-postgres-connect-gce`.        |
