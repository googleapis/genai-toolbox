---
title: cloud-sql-mysql-connect-gce
type: docs
weight: 12
description: Connect a Cloud SQL for MySQL instance to a Google Compute Engine VM — validates network reachability, recommends a connection method, and emits setup steps plus an optional code snippet.
---

## About

The `cloud-sql-mysql-connect-gce` tool helps an agent (or user) wire up a
Cloud SQL for MySQL instance to a Google Compute Engine VM. It:
...
tools:
  connect_to_gce:
    ...
    description: Help me connect a Cloud SQL MySQL instance to a GCE VM.
```
...
| ----------- | :------: | :----------: | ------------------------------------------------ |
| type        |  string  |     true     | Must be `cloud-sql-mysql-connect-gce`.           |
